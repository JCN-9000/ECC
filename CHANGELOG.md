...
- Port `.opencode/plugins/ecc-hooks.ts` to the OpenCode V2 plugin API (`Plugin.define` with `ctx.tool`, `ctx.shell`, `ctx.session`, `ctx.permission`, and `ctx.event`). The V1 default-export-a-function shape failed to load on OpenCode 2.x and showed the plugin as failed in `opencode plugin list`.
- Keep the plugin loadable when `.opencode/plugins/lib` is missing or partially installed: `plugins/lib/process.ts` is now imported lazily, as the changed-files store already was, so a broken install degrades the affected hooks instead of throwing during module evaluation (#2530).
- Stop `runProcess` from killing the host process with an unhandled `EPIPE` when a child exits before reading its stdin (or an aborted child tears down its pipes).

### Added

- `plugins/clv2-observe.ts` and `plugins/ponytail-v2.ts` V2 plugins (session observation and the Ponytail coding-discipline guard).
- `plugins/lib/process.ts` subprocess helper used by the hooks in place of the removed V1 `$` shell template.

### Changed

- Reconnect file-edit behavior to the V2 `filesystem.changed` event: Prettier auto-format, the console.log warning, and changed-files tracking all run from it.
- Register hooks through the V2 event stream and hook points; guard setup with a module-level flag so the plugin barrel and the file entry point cannot wire the same hooks twice.
- Drop the redundant `"plugin": ["./plugins"]` entry from `opencode.json`; the directory is auto-discovered and the entry loaded the plugin a third time.

### Removed

- The `file.edited`, `todo.updated`, and `file.watcher.updated` handlers, which do not exist in the V2 event stream.
- The plugin-level `tool` registration for `changed-files` and `dependency-analyzer`; both load from the `tools` directory on their own.
