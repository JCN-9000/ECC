...
// Files ECC already installed are only updateable when the previous install
// recorded them as managed; anything else is user-owned and left alone.
function recordManagedInstall(installStatePath, options) {
  writeInstallState(installStatePath, createInstallState({
    adapter: { id: 'test-install', kind: 'project' },
...
function runTests() {
  ...
  if (test('--update copies only missing or outdated files', () => {
    ...
    try {
      ...
      // A prior install already owns these destinations, so --update is free to
      // refresh the outdated ones instead of treating them as user-owned.
      recordManagedInstall(installStatePath, {
        targetRoot,
  ...
  if (test('--update dry-run lists only files that would be copied', () => {
    ...
    try {
      // Install first so ECC owns the destination; an unrecorded file is
      // user-owned and is reported as skipped for a different reason.
      const firstInstall = run(['--profile', 'core', '--target', 'opencode', '--no-hooks'], { cwd: projectDir, homeDir });
      assert.strictEqual(firstInstall.code, 0, firstInstall.stderr);

      const destPath = path.join(homeDir, '.config', 'opencode', 'commands', 'aside.md');
      assert.ok(fs.existsSync(destPath), 'sanity check: the first install should write the file');

      const result = run(
        ['--profile', 'core', '--target', 'opencode', '--no-hooks', '--update', '--dry-run'],
        { cwd: projectDir, homeDir }
      );
      assert.strictEqual(result.code, 0, result.stderr);
